 ...
 * @return bool|string
 */
function cleana_the_excerpt( $trim_character_count = 0 ) {
	$post_ID = get_the_ID();
 ...
 */
function cleana_excerpt_more( $more = '' ) {
 ...
 */
function cleana_the_post_pagination( $current_page_no, $posts_per_page, $article_query, $first_page_url, $last_page_url, bool $is_query_param_structure = true ) {
	...
	<div class="mt-0 md:mt-10 mb-10 lg:my-5 flex items-center justify-end posts-navigation">
		...
		echo paginate_links( [
			...
			'total'     => $total_pages,
			'prev_text' => __( 'Prev', 'cleana' ),
			'next_text' => __( 'Next', 'cleana' ),
		] );
 ...
 */
function cleana_is_uploaded_via_wp_admin( $gravatar_url ) {
 ...
 * @return bool
 */
function cleana_has_gravatar( $user_email ) {

	$gravatar_url = get_avatar_url( $user_email );

	if ( cleana_is_uploaded_via_wp_admin( $gravatar_url ) ) {
		return true;
	}
